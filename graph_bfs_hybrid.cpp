...
std::vector<int> bfs_mpi(const Graph& graph, int source,
    ...
    while (!current_frontier.empty()) {
        ...
        std::vector<std::vector<int>> thread_local_next(max_threads);

        #pragma omp parallel
        {
            int tid = omp_get_thread_num();
            auto& local_buf = thread_local_next[tid];
            ...
            for (int i = 0; i < frontier_size; ++i) {
                ...
                for (int neighbor : graph.adj[current]) {
                    if (neighbor >= lo && neighbor < hi) {
                        // Atomic CAS to prevent race conditions across threads
                        int expected = -1;
        ...
        for (auto& buf : thread_local_next) {
